...
{	int count;
	...
	// Printing the queens in the correct position.
	
	for(count = 1; count <= num_queens; ++count) {	

		// Figures out the corridantes for the x and y "boxes" and divides them both in half to get the middle of the box.
	...
	// If the F1 key is pressed exit the program.
...
// Draws the board with lines, using the ACS constants in ncurses.
//
void board(WINDOW *win, int starty, int startx, int lines, int cols,
	   int tile_width, int tile_height)
{	int endy, endx, i, j;
